...
class AndroidCamFeed:
    ...
    __bytes = ''
    __noStreamCount = 1
    __loadCode = cv2.IMREAD_COLOR if sys.version_info[0] > 2 else cv2.IMREAD_COLOR

    def __init__(self, host):
        self.host_string = 'http://' + host + '/video'
        try:
            AndroidCamFeed.__stream = urllib2.urlopen(self.host_string, timeout=3)
            AndroidCamFeed.__isOpen = True
        except (socket_error, urllib2.URLError) as err:
            print "Failed to connect to stream. \nError: " + str(err)
            self.__close()
        t = threading.Thread(target=self.__capture_feed)
        t.start()

    def __capture_feed(self):
        while AndroidCamFeed.__isOpen:
            new_bytes = AndroidCamFeed.__stream.read(1024)
            if not new_bytes:
                self.__no_stream()
                continue
            AndroidCamFeed.__bytes += new_bytes
